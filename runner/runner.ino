...
void loop() {
  M5.update(); // Always update M5 buttons

  if (M5.BtnA.isPressed()) {
    Serial.println("Button A pressed, starting recording...");

    // Record while Button A is pressed
    while (M5.BtnA.isPressed()) {
      mic_record_task();
      M5.update();
    }

    Serial.println("Button A released, sending audio...");

    // Convert the recorded audio into text
    String userFact = sendAudioToApi(adcBuffer);
